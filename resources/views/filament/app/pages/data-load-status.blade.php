<x-filament::page :wire:poll.1000ms="$this->shouldPoll ? 'pollStatus' : false">
    ...
    @if ($dataLoad)
        ...
        <div class="space-y-6 max-w-md">
            ...
            <div class="bg-white shadow rounded-lg p-6 border border-gray-200 space-y-2">
                ...
                <p class="text-sm text-gray-700"><strong>Worksheet:</strong> {{ $dataLoad->worksheet_name ?? 'N/A' }}
                </p>
                ...
                @if ($dataLoad->row_count)
                    <p class="text-sm text-gray-700"><strong>Total
                            Rows:</strong> {{ number_format($dataLoad->row_count) }}
                    </p>
                @endif
            ...
            @if ($dataLoad->validation_status === 'completed')
                <div class="bg-white shadow rounded-lg p-6 border border-gray-200 space-y-4">
                    ...
                    <div class="flex flex-wrap gap-3 pt-2">
                        ...
                        <x-filament::button wire:click="viewData">
                            View uploaded data
                        </x-filament::button>
                    </div>
                </div>
    ...
    @else
        <div class="bg-white p-6 rounded-lg shadow text-center text-gray-600">
            No data loads have been uploaded yet.
        </div>
    @endif
</x-filament::page>
